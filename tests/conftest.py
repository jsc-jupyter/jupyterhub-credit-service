...
from jupyterhub import crypto, orm, scopes
...
from jupyterhub_credit_service import CreditsAuthenticator, CreditsSpawner
...
def users(app):
    ...
    u1 = add_user(app.db, app, name=new_username())
    u2 = add_user(app.db, app, name=new_username())
    yield [u1, u2]
...
def count_db_executions(request, record_property):
    if "app" in request.fixturenames:
        ...
        initial_count = getattr(app, "db_query_count", 0)
        yield
